...
import { accessHref, ApiError, listPassengers, setTripTravellers } from "../api";
  ...
  return (
    <section className="trip-traveller-picker">
      <div className="trip-traveller-heading">
        ...
        <a className="quiet-link" href={profileHref(tripId)}>Manage</a>
      </div>
      ...
      {!sessionCredential ? (
        ...
        <a className="traveller-picker-empty" href={profileHref(tripId)}>
...
export function travellerProfileHref(tripId: string, passengerId?: string): string {
  const url = new URL(accessHref("/profile", tripId), window.location.origin);
  url.searchParams.set("tab", "travellers");
  if (passengerId) url.searchParams.set("traveller", passengerId);
  return `${url.pathname}${url.search}${url.hash}`;
}

function profileHref(tripId: string): string {
  return travellerProfileHref(tripId);
}
