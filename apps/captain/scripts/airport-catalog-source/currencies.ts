/**
 * ISO 3166-1 alpha-2 → ISO 4217, for the currency on each airport.
 *
 * Only two values are load-bearing: `suggestedTripCurrency` keeps a currency
 * only when `isSupportedTripCurrency` accepts it, and Captain supports USD and
 * GBP alone. Everything else here is filtered out before it reaches anyone, so
 * a long-tail entry being stale costs nothing — but GB and US must be right.
 */
export const COUNTRY_CURRENCIES: Readonly<Record<string, string>> = {
  ...
  PM: "EUR", PR: "USD", PS: "ILS", PT: "EUR", PW: "USD", PY: "PYG",
  QA: "QAR",
  ...
  TC: "USD", TD: "XAF", TF: "EUR", TG: "XOF", TH: "THB", TJ: "TJS", TL: "USD",
