...
- A newly confirmed Trip tracks alongside existing Trips until the three-Trip
  ...
  a receipt.
- The confirmed Trip currency is locked. Never convert, combine, or compare
  fares in different currencies.
- Use `manage_trip` for pause, resume, refresh, cancel, or complete. Searches
  are asynchronous and manual refreshes may be limited.
- Only describe offers returned by `get_trip`. Those passed two independent
  web checks. Never claim the set is exhaustive.
